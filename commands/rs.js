...
module.exports = {
  ...
  execute(message, args) {
    ...
    async function getScore(id) {
      ...
      const apiURL =
        "https://osu.ppy.sh/api/v2/users/" + id + "/scores/recent?limit=1";
      const AuthStr = "Bearer " + osuKey.access_token;
      let response = await axios
        .get(apiURL, { headers: { Authorization: AuthStr } })
        .catch((error) => {
      ...
      getScore(playerData.id).then((score) => {
        ...
        getMaxPP(score[0].beatmap.id).then((mapFile) => {
          ...
          const data = {
            ...
            thumbnail: "https://b.ppy.sh/thumb/"
              .concat(score[0].beatmapset.id)
              .concat(".jpg"),
          };
          ...
          const maxPp = osu
            .ppv2({ map: parser.map, mods: osuMods })
            .toString()
            .split(" ");
          ...
          if (data.pp == null) {
            ...
            embed.spliceFields(4, 1, {
              name: "pp",
              value: data.pp.toFixed(2) + "/" + maxPp[0],
              inline: true,
            });
          }
