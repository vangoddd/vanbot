        ...
        if (!args.length) {
            axios
                ...
                .then((response) => {
                    ...
                    // send(
                    //   `\`\`\` ${response.data[0].code} (${response.data[0].tags})\nShowing 1 of ${response.data[0].total} Sauces\`\`\``
                    // );
                })
                .catch((error) => {
                    console.log(error);
        ...
        // if there is an argument
            ...
            // !hotsauce add <code> <tag>
                ...
                axios
                    ...
                    .then((response) => {
                        send(
                            "Sauce added to db. Thanks for your contribution <3"
                        );
                    })
                    .catch((error) => {
                        console.log(error);
            ...
            // !hotsauce <tag> <number>
                ...
                axios
                    ...
                    .then((response) => {
                        ...
                        const msg = [];
                        for (let i = 0; i < response.data.length; i++) {
                            msg.push(response.data[i].code);
                        }

                        msg.push(
                            `\nShowing ${response.data.length} of ${response.data[0].count} Sauces tagged with "${response.data[0].tags}"`
                        );
