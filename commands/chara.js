...
module.exports = {
  ...
  execute(message, args) {
    ...
    async function getSearch() {
      ...
      let response = await axios.get(searchUrl).catch((error) => {
        console.log(error);
      });
    ...
    async function getChara(id) {
      ...
      let response = await axios.get(charaUrl).catch((error) => {
        console.log(error);
      });
      return response.data;
    }
    ...
    function navigateEmbed(reaction, embed, length) {
      ...
      newEmbed.setImage(charImage[pos]).setFooter(`${pos + 1} / ${length}`);
        ...
        getCharImage(chara.mal_id).then((imgList) => {
          ...
          if (chara.animeography.length > 0) {
            embed.setDescription(
              `[${chara.animeography[0].name}](${chara.animeography[0].url})` +
                "\n" +
                voiceActor
            );
          } else if (chara.mangaography.length > 0) {
            embed.setDescription(
              `[${chara.mangaography[0].name}](${chara.mangaography[0].url})` +
                "\n" +
                voiceActor
            );
          }
